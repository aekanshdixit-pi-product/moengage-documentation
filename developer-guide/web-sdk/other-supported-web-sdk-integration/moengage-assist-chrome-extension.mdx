...
The MoEngage Assist Chrome extension is a dedicated debugging tool designed to help developers and marketers monitor and troubleshoot their MoEngage Web SDK implementation. Install  the extension to  validate and debug your Web SDK integration directly from your browser.

With this extension, you can: 

- Verify service worker registration.
- Confirm push token generation.
- Inspect event payloads in real time.
...
- **Campaign:** Flags campaign-delivery errors (for example, attempting to track a card ID that has not been received or has been deleted).
...
<Tabs>
  <Tab title="Basic Details">
    ...
    - **SPA mode enabled:** Indicates whether Single Page Application tracking is enabled for dynamic content loading.
    - **Shopify:** Displays the operational status of your Shopify integration.
    - **Billing type:** Outlines your active subscription model (for example, _Subscriber based billing_).
...
<Tabs>
  ...
  <Tab title="User">
    ...
    - **User identity:** Indicates the current identification state of the user within the MoEngage ecosystem (for example, _Anonymous_ or _Identified_).
    - **Device connection:** Validates the browser or hardware profile mapping.
    ...
    - **Session details:** Provides temporal and sequential data regarding the user's active browsing period.
      ...
      - **Number of sessions:** The cumulative count of sessions associated with this specific user or device (for example, _93_).
    ...
    **Warning:** Manually clearing local storage  resets the MoEngage SDK session and break event tracking. Always utilize the destroySession() method before clearing localStorage.
  </Tab>
</Tabs>
...
<Info>
  ...
  - If a specific module or feature has not been integrated within your Web SDK implementation, MoEngage Assist displays a descriptive error state to assist with troubleshooting. For example, if the Web Personalization module is missing, the extension indicates: _"Personalize not integrated. To start seeing campaigns, please integrate the Personalize SDK on your website."_
</Info>
