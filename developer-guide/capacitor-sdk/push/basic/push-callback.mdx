...
In the case of iOS, you would always receive the key-value pairs with respect to clicked action in `clickedAction` property. Refer to this [Notification Payload](/developer-guide/ios-sdk/push/advanced/custom-notification-handling)  for knowing the iOS notification payload structure.