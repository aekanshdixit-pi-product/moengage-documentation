...
| library\_add\_check | **Prerequisites**   To enable push notifications:   1. **Integrate the MoEngage iOS SDK**: Use CocoaPods, Swift Package Manager (SPM), or manual framework import as detailed in the [MoEngage documentation](https://developers.moengage.com/hc/en-us/sections/6993218591636-Basic). 2. **Configure APNs credentials in the MoEngage UI**: 	1. MoEngage requires either an APNs Authentication Key (.p8) or an [APNs Certificate](https://developers.moengage.com/hc/en-us/articles/4403944011028-APNS-Certificate-PEM-file) (.pem) to communicate with Apple Push Notification Service (APNs). 	2. Recommendation: Use an APN's Authentication Key because it is non-expiring. Refer to the [APNs Authentication Key Setup Guid](https://developers.moengage.com/hc/en-us/articles/8484447635348-APNS-Authentication-Key#apns-certificate--0-0)e. 3. Ensure you have added **Push Notifications** capability in the main target by navigating to **Target** -> **Signing & Capabilities**. |
| --- | --- |
...
2. Integrate MoEngageRichNotification:
...
3. To set up an App Group for seamless communication between your main app and NSE, perform the following steps:
	1. Enable App Groups:
		1. For both your main application target and your NSE target, navigate to **Signing and Capabilities**.
		2. Click **+ Capability** and select **App Groups**.
	2. Create App Group ID:
		1. Create a new App Group ID (for example, group.com.yourcompany.appname).
		2. Crucially, ensure this exact App Group ID is enabled for both the main app and the NSE targets.
...
4. Configure the NSE Code by replacing the entire content of the generated `NotificationService.swift` file with the following:
...
arrow\_drop\_down **How to resolve App Delegate Swizzling conflicts?**
...
Before sending a test notification, first verify the integration status on your MoEngage dashboard.

Navigate to the iOS Push section of your profile and confirm the following:
...
For more information, refer to [iOS Push Notifications Integration Validation](https://help.moengage.com/hc/en-us/articles/360052433591).
...
* [**Notification Content Extension**](https://developers.moengage.com/hc/en-us/articles/4403956104084-Push-templates): Renders a custom UI for the notification. Use this to create interactive experiences, such as image carousels or custom layouts.

## Actionable Buttons

Embed buttons within a notification to allow users to perform tasks directly from the notification interface, such as "Reply" or "Archive." For more information, refer [here](https://developers.moengage.com/hc/en-us/articles/4403961980308-Actionable-Notifications).
...
arrow\_drop\_down **Why are my push notification clicks not being tracked, or why are my deep links not opening?**
...
arrow\_drop\_down **Why is rich media (images, GIFs) not showing in my notifications, or why is impression tracking failing?**
...
arrow\_drop\_down **Why are push notifications not being delivered?**
...
arrow\_drop\_down **Why is the** `**didRegisterForRemoteNotificationsWithDeviceToken**` **delegate method not called?**
...
arrow\_drop\_down **Why are provisional push notifications not appearing?**
