...
MoEngageSDKConfig* sdkConfig = [[MoEngageSDKConfig alloc] initWithAppID:appID];
sdkConfig.enableLogs = true;
```
...
* Make sure that the [SDK initialization](https://developers.moengage.com/hc/en-us/articles/4403905438228) is done properly. Check if the App ID provided in the initialization method is correct.
...
* Make sure you have implemented user unique ID tracking properly. Please set [**User Attribute Unique ID**](https://developers.moengage.com/hc/en-us/articles/4403905883796) **unique** to your users that are logged, don't hardcode this value.
* Make sure [resetUser](https://developers.moengage.com/hc/en-us/articles/4403905883796) method is called on the logout feature of the App so that SDK can differentiate between the users.
...
* Set Build Configuration correctly and test in the correct environment. Refer [link](https://developers.moengage.com/hc/en-us/articles/4403905438228-SDK-Initialisation#test-live-environment-0-3) for more info.
...
* Ensure that you are calling MoEngage SDK methods in callback methods which are called on receiving remote notifications. Refer [link](https://developers.moengage.com/hc/en-us/articles/23448115571732-Custom-Notification-Handling#h_01H92ZQDWJWX6615STBKN7NRYJ) to know more.
...
* Make sure you have implemented the [Rich Notification](https://developers.moengage.com/hc/en-us/articles/43960004257428-iOS-Push-Integration-Tutorial#h_01K45FKD0XDH4X776R47JA04GJ) feature correctly.
...
* Make sure **handleInAppMessage** method is called wherever In-App is displayed - [link](https://developers.moengage.com/hc/en-us/articles/4404155127828-In-App-NATIV-SDK-Version-6-0-0-and-above-#how-to-show-in-app-message--0-1)
* In-App only came once and can’t see/test it again. Then it might be a case where inApp to be shown may not pass a set of rules which we have kept in SDK to make sure the user experience isn't hindered. Refer the [link](https://developers.moengage.com/hc/en-us/articles/4404155127828-In-App-NATIV-SDK-Version-6-0-0-and-above-#in-app-messaging-rules-0-13) to know about these rules.
...
* [Enable SDK logs](#enable-sdk-logs-0-0) to get more details on why inApp didn't show up and share it with MoEngage Team.
