...
description: "Build custom UI for card campaigns using the MoEngage Cards data APIs."
---
...
To fetch cards eligible for display for a specific category, use the `getCardData` API:

<CodeGroup>

```swift Swift wrap
MoEngageSDKCards.sharedInstance.getCardData(for: "CATEGORY", appID: "YOUR Workspace ID") { cardsData, accountMeta in
...
```objective-c Objective C wrap
[[MoEngageSDKCards sharedInstance] getCardDataFor:@"CATEGORY" appID:@"YOUR Workspace ID" completionBlock:^(MoEngageCardsData * _Nullable cardsData, MoEngageAccountMeta * _Nullable accountMeta) {
...
<Warning>
  The older `getCards(forCategory:forAppID:withCompletionBlock:)` and `getCardsData(forAppID:withCompletionBlock:)` APIs are deprecated. Use `getCardData(for:appID:completionBlock:)` instead.
</Warning>
