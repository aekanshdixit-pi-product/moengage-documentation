...
<AccordionGroup>
  <Accordion title="What causes a crash when tracking a Personalization experience click?">
    When calling experienceClicked(), the experienceAttribute dictionary is strictly validated. The `experienceAttribute` dictionary must contain both `"cid"` and "experience.". Missing any of these keys, or passing an invalid Workspace ID, will result in a crash in Debug mode.
  </Accordion>
</AccordionGroup>