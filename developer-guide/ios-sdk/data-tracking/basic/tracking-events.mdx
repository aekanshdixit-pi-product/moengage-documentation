...
<AccordionGroup>
  <Accordion title="Why did my app crash with a fatal error about an empty event attribute name?">
    In Debug mode, the SDK strictly validates event attributes. Passing an empty string ("") as an attribute name is fatal. You must always provide a valid, non-empty string for attribute names.
  </Accordion>

  <Accordion title="Why is the app crashing due to unsupported event attribute values?">
    To protect backend data integrity, the SDK throws an exception in Debug mode if you pass unsupported attribute values. Common triggers include:

    - Passing null (NSNull()).
    - Passing custom models, UI elements (UIColor, UIImage).
    - Passing invalid numbers like Double.nan or Infinity.
    - Passing empty Arrays or Dictionaries.
      Passing Date or MoEngageGeoLocation objects inside an Array or as a Dictionary value (these must be passed directly, not nested).
  </Accordion>
</AccordionGroup>