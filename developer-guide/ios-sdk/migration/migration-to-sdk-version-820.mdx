...
For info on the new initialization methods, refer to this [link](https://developers.moengage.com/hc/en-us/articles/4403905438228).
...
For info on the new DataCenter value, refer to this [link](https://developers.moengage.com/hc/en-us/articles/11290950997652-Data-Center).

# App Target: Setting App Group ID

App Group ID for App target was earlier set using `setAppGroupID:`, which is no longer supported. Make use of the MOSDKConfig to configure the same in the latest SDK versions(Refer to the [link](https://developers.moengage.com/hc/en-us/articles/4403905438228) for more info)
...
| MoEngage.sharedInstance().setUserLocationLatitude   (userLocationLat, andLongitude: userLocationLng) | MOAnalytics.sharedInstance.setLocation(MOGeoLocation(withLatitude: userLocationLat, andLongitude: userLocationLng)) |
| MoEngage.sharedInstance().setUserAttributeISODateString   ("2020-01-12T18:45:59Z", forKey: "DateAttr2") | MOAnalytics.sharedInstance.setUserAttributeISODate("2020-01-12T18:45:59Z", withAttributeName: "Date Attr 2")e |
| MoEngage.sharedInstance().setUserAttributeTimestamp   (NSDate().timeIntervalSince1970, forKey:"DateAttr3") | MOAnalytics.sharedInstance.setUserAttributeEpochTime(663333, withAttributeName: "Date Attr 3") |
| MoEngage.sharedInstance().setUserAttributeLocationLatitude   (12.98798, longitude: 34.98789, forKey:"location\_attribute\_name") | MOAnalytics.sharedInstance.setLocation(MOGeoLocation.init(withLatitude: 72.90909, andLongitude: 12.34567), withAttributeName: "attribute name") |

For more information on tracking user attributes methods, refer to this [doc](https://developers.moengage.com/hc/en-us/articles/4403905883796).
...
| //Set messaging delegate   MOMessaging.sharedInstance().messagingDelegate = self | //Set messaging delegate    MOMessaging.sharedInstance.setMessagingDelegate(self) |

# RealTimeTrigger and Inbox Module

From SDK version 8.0.0, RealTimeTrigger and Inbox Module are separated from the MoEngage-iOS-SDK.Hence, they must be integrated explicitly. Refer to the [link](https://developers.moengage.com/hc/en-us/articles/11381968015636-Real-Time-Triggers) for the document.
...
For more info on the Geofence feature, refer to this [doc](https://developers.moengage.com/hc/en-us/articles/11381992240148-Location-Triggered).
...
For more info on the InApp feature, refer to this [doc](https://developers.moengage.com/hc/en-us/articles/11291104911380-In-App-Nativ).
...
| SDK Version 7.0.0 | SDK Version 8.2.0 |
| --- | --- |
| MoEngageCards.sharedInstance.pushCardsViewController   (toNavigationController: self.navigationController!) | MOCards.sharedInstance.pushCardsViewController   (toNavigationController: self.navigationController!) |
...
For more info on the Cards feature, refer to this [doc](https://developers.moengage.com/hc/en-us/articles/11291315865876-Cards-in-iOS).
