...
<Warning>
  **Live Activity tracking validations**
  ...
  - **Track before initialization.** Live Activity tracking methods must be called only once the SDK is initialized. Calling them earlier — for example, before the initializer runs in `application(_:didFinishLaunchingWithOptions:)` — throws a fatal exception and crashes the app in `DEBUG` builds. In Release and TestFlight builds, the call is dropped silently and logged.
