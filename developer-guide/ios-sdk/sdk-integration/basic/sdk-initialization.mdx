...
<AccordionGroup>
  <Accordion title="Why does the app crash on launch, related to auto-initialization and info.plist?">
    If you rely on auto-initialization, the SDK automatically reads the info.plist file on launch. If this file contains malformed data, incorrect data types, missing required fields, or corrupt JSON, the SDK will fail to parse it and throw an exception.
  </Accordion>

  <Accordion title="Why did initializing the default SDK instance result in a fatal error?">
    Calling MoEngage.sharedInstance.initializeDefaultInstance() will crash in Debug mode if the SDK cannot find a valid configuration. This happens if there is no valid info.plist present and you did not provide a valid configuration object. It will also crash if you explicitly pass a configuration object with an empty App ID.
  </Accordion>
</AccordionGroup>