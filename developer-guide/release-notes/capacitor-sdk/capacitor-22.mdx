...
| API | Platform |
| :-- | :-- |
| `getSelfHandledInApps(options: { appId: string; }): Promise<MoEInAppSelfHandledCampaignsData>;` | Hybrid |
| `registerForProvisionalPush(options: { appId: string; }): void;` | Hybrid |
| `addListener(eventName: "logoutCompleted", listenerFunc: (data: MoELogoutCompleteData) => void): void;` | Hybrid |
| `initialiseDefaultInstance(application: Application, sdkState: SdkState? = null)` | Android Native |
| `intializeDefaultInstance(withAdditionalConfig config: MoEngageSDKDefaultInitializationConfig)` | iOS Native |
