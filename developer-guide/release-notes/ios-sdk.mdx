...
<Badge color="orange">Fixes:</Badge>\
Fixed a storage deduplication issue that could affect developers upgrading from v7.x to v10.x. The migration steps have been corrected to handle this properly.
...
<Badge color="green">What's New:</Badge>\
Added support for initializing the default SDK instance with configuration data in the application's Info.plist. This is primarily intended for cross-platform frameworks (such as React Native, Flutter, and Cordova) that use this mechanism for SDK initialization and is not exposed as a native SDK API.
...
- Added missing accessibility data inside the metadata dictionary of cards for cross-platform frameworks.
...
<Badge color="orange">Improvements:</Badge>
...
<Badge color="green">What's New:</Badge>\
Added Kotlin Multi-Platform based event trigger condition evaluator (`MoEngageKMMConditionEvaluator`), enabled by default. This adds approximately 3.3 MB to the arm64 device binary size.
