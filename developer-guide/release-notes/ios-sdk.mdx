...
<Info>
  The SDK now throws an exception that crashes the app in debug mode when required attributes are missing for a Personalize experience event, or when an invalid workspace is configured. To disable this behavior, call [`disableIntegrationValidator()`](https://moengage.github.io/ios-api-reference/Classes/MoEngageSDKCore.html#/c:@M@MoEngageCore@objc(cs)MoEngageSDKCore(im)disableIntegrationValidator). This crash is limited to Xcode debug builds and does not affect TestFlight or App Store builds. For more information, see [Personalize SDK](/developer-guide/ios-sdk/personalize/personalize-sdk).
</Info>
...
- Fixed a memory leak that occurred when the application was running in the background.
...
- Fixed a crash that occurred when testing HTML nudge templates via test push.
...
- **Segment Re-evaluation:** Trigger-time segment re-evaluation ensures in-app messages are shown only to users who are eligible at trigger time. For more information, see [Re-evaluate Campaign Eligibility](/user-guide/campaigns-and-channels/in-app-message/create/create-in-app-campaign#re-evaluate-campaign-eligibility).
...
<Info>
  The SDK now throws an exception that crashes the app in debug mode when Transactional Live Activity tracking is set up incorrectly. To disable this behavior, call [`disableIntegrationValidator()`](https://moengage.github.io/ios-api-reference/Classes/MoEngageSDKCore.html#/c:@M@MoEngageCore@objc(cs)MoEngageSDKCore(im)disableIntegrationValidator). This crash is limited to Xcode debug builds and does not affect TestFlight or App Store builds. For more information, see [Transactional Live Activity](/developer-guide/ios-sdk/push/optional/Transactional-Live-Activity).
</Info>
...
- Fixed template backup subtitle overriding rich push.
...
- Added option to remotely update the data center host. For more information, see [Data Center](/developer-guide/ios-sdk/sdk-integration/basic/data-center).
...
- Fixed border rendering on resizable in-app messages.
...
- Fixed an in-app condition evaluator thread hang/crash.
...
<Warning>
  The legacy `logNotificationReceived` API (without a completion callback) is deprecated. Migrate to `logNotificationReceived(completionCallback:)` to ensure reliable impression tracking.
</Warning>
...
- Added accessibility support so GIFs and images remain visible in in-app nudges.
- Fixed a random crash in in-app messages on removing context.
...
- Updated the order of event tracking for the `Offering Clicked` event.
...
- Added missing accessibility data inside the metadata dictionary of cards for hybrid platforms.
...
- Added APIs for managing and tracking Live Activities. For more information, see [Broadcast Live Activities for iOS Push Campaigns](/user-guide/campaigns-and-channels/mobile-push/notification-features-and-behavior/broadcast-live-activities-for-ios-push-campaigns).

<Info>
  The SDK now throws an exception that crashes the app in debug mode when Broadcast Live Activity tracking is set up incorrectly. To disable this behavior, call [`disableIntegrationValidator()`](https://moengage.github.io/ios-api-reference/Classes/MoEngageSDKCore.html#/c:@M@MoEngageCore@objc(cs)MoEngageSDKCore(im)disableIntegrationValidator). This crash is limited to Xcode debug builds and does not affect TestFlight or App Store builds. For more information, see [Broadcast Live Activity](/developer-guide/ios-sdk/push/optional/broadcast-live-activity).
</Info>
...
- Fixed the provisional opt-in device attribute not being tracked after 24 hours.
...
- Added accessibility support for in-app campaigns.

<Badge color="orange">Fixes:</Badge>

- Fixed an app crash that occurred when `rootViewController` was nil during an in-app campaign display.
...
<Card title="Release Summary">
  | SDK Version | Modules | Dependencies |
  | :-- | :-- | :-- |
  | 9.24.0 | <ul><li><Badge color="green" size="sm">Messaging</Badge></li><li><Badge color="green" size="sm">Inbox</Badge></li><li><Badge color="green" size="sm">InApp</Badge></li><li>Core</li><li>Cards</li><li>Geofence</li><li>RichNotification</li><li>RealTimeTrigger</li></ul> | swift = 5.0.0<br />iOS = 11.0<br />tvOS = 11.0 |
</Card>
...
- Fixed a crash that occurred when switching App IDs.
