...
<Warning>
  Do not remove `InitializationProvider` from the merged manifest. If it is removed (for example, with a `tools:node="remove"` rule), the SDK can't track foreground or background transitions, which breaks session tracking and several push and in-app features.
</Warning>
