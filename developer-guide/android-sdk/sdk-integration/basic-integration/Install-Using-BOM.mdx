...
Replace `<bomVersion>` with the latest version number. To find the latest BOM version, refer to the [maven-central badge](https://central.sonatype.com/artifact/com.moengage/android-bom) above or check the [Android SDK release notes](/developer-guide/release-notes/android-sdk).

## Add MoEngage Modules

Once the BOM is configured, include the specific MoEngage modules required for the application. Version numbers are not required for these dependencies; the BOM manages them.

<Info>
  **Required core module** — Always include `com.moengage:moe-android-sdk`. It is not pulled in transitively by the BOM and is needed for core SDK initialization and data tracking.
</Info>
...
dependencies {
    ...
    // Core SDK - Required for initialization and data tracking
...
The MoEngage SDK depends on the **lifecycle-process** library, and the latest version of **lifecycle-process** depends on the **androidx.startup:startup-runtime** library. The `InitializationProvider` is a `ContentProvider` declared by `startup-runtime` in the merged manifest; it lets `lifecycle-process` initialize automatically at app start so the SDK can observe the application lifecycle.

<Warning>
  Do not remove `InitializationProvider` from the merged manifest. If you remove it (for example, with a `tools:node="remove"` rule), the SDK can't track foreground or background transitions, which breaks session tracking and several push and in-app features.
</Warning>

If you add other Initializers using `startup-runtime`, make sure the Initializer for the `lifecycle-process` library is also added. Refer to the [App Startup documentation](https://developer.android.com/jetpack/androidx/releases/lifecycle#2.4.0).
...
- **Compatibility:** The BOM ensures that all integrated modules work together, preventing runtime crashes caused by mismatched library versions.
- **Cleaner Configuration:** Maintains a clean `build.gradle` file with a single source of truth for the SDK version.

## Next Step

Continue to [SDK Initialization](/developer-guide/android-sdk/sdk-integration/basic-integration/sdk-initialization) to initialize the SDK in your Application class.