---
title: "Release checklist 11.x.x"
description: ""
---
...
* SDK is initialized in the `onCreate()` of the Application class on the Main Thread.
* Disable logging for release build if enabled.
* If using SDK version `11.0.00` and above, enable Java 8 target and source compatibility.

# Data Tracking

* Install/Update Differentiation
* Exhaustive Events and Attributes tracked based on use-cases.
...
* Logout method called when the user is logged out of the application
...
* Check Firebase receiver in the `AndroidManifest.xml` file. Make sure only 1 receiver is added in the manifest file.
...
* Push Payload passed on to the SDK
	+ Token Registration and Payload handled by SDK
	+ SDK's Mi receiver added in the manifest
	+ Token change listener registered if required.
...
* Call `getSelfHandledInApp()` and register the callback listener if required.
