...
* Analytics: Updated destroy\_session() method to handle parallel user logouts
* Analytics: Added support to store third-level domain cookies
* Analytics: Added expiry date to moe\_uuid cookie
...
* OSM: Added Self Handled OSM support. Follow the integration doc [here](https://developers.moengage.com/hc/en-us/articles/41892522947732-Self-Handled-On-Site-Messaging)
...
* Data Tracking: Tracking moe\_aid in all report calls if present
...
* NPM Package: handled cards on\_load callback in case when cards initialisation takes time
...
* Shopify: Added cart\_token request param in Shopify multi-ID enabled clients
...
* CARDS: prev\_sync\_card\_id added in get call
...
* Push: Service worker issue of tracking impression n click fixed
...
* Web Personalisation: Generating unique\_id for first load
* Shopify: tracking cart\_token even if web push is disabled.
...
* tracking `migrated\_user` for push migration case in subscribe event
...
* Using device\_uuid from cookies in case of AMP
...
* added unique\_id check in OSM API calls
