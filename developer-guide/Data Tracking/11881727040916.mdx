...
Every `trackEvent()` method call expects 2 parameters, event name and event attributes using `MoEProperties` an instance that represents attributes of the event. Add all the additional information which you think would be useful for segmentation while creating campaigns.  
...
MoECapacitorCore.trackEvent({ eventName: "Purchase", eventAttributes: properties });
```
...
To mark an event as non-interactive set `isNonInteractive` property of the `MoEProperties` to `true` as shown below:
