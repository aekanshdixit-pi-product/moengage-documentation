...
The MoEngage On-Site Messaging (OSM) Template API allows you to programmatically manage your web and app-site messaging templates. You can use this API to upload custom HTML templates created outside the MoEngage ecosystem, making them available for your OSM campaigns. 
...
The OSM Template API includes the following collection of endpoints:
...
<Accordion title="What are the rate limits for this API?" icon="gauge-high">
  The rate limit for Create, Update, and Search operations is **100 Requests Per Minute (RPM)**.
</Accordion>

<Accordion title="Is pagination mandatory for searching?" icon="list-ol">
  Yes. Effective **November 15, 2025**, all search requests must include the `page` and `entries` parameters. The maximum number of entries per page is 15.
</Accordion>
