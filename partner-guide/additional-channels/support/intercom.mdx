...
4. In the **\+ New workflow** list, click **Create from scratch**.
   ![](/images/partner_41411186745492.png)
   The **Create from scratch using the following triggers** dialog box appears.
5. Now, select your desired workflow trigger (for example, click **Ticket-only** category on the left, and then click **When a ticket is created trigger**).
   ![](/images/partner_41390933738516.png)
   Now, you must configure the trigger.
6. In the **When a ticket is created** panel on the workflow canvas, set the following: **Channels**: Specify which channels the trigger will apply to (for example, Web, iOS and so on). **Filters**: Select the workflow to run for **Users** , **Leads** , or both.
7. Click **\+ Add step** to add the data connector action.
8. From the **Choose what's next** menu, search or select **Data connector**.
   ![](/images/partner_41456677398676.png)
9. Select the specific connector you created earlier.
   ![](/images/partner_41456677401364.png)
...
3. On the **Conversations** page, click **\+ Create attribute** in the upper-right corner to begin the setup process.
   ![](/images/partner_41411186747540.png)
   The **Create a new attribute** dialog box appears.
...
5. Click **Save**.
   ![](/images/partner_41411252387860.png)
...
1. In the left navigation bar of your Intercom dashboard, click the **Settings** icon <img src="/images/partner_41376635681684.png" /> in the lower-left navigation bar.
2. On the **Home** page, under the **Integrations** section, click **Developer Hub**.
   ![](/images/partner_41411166801300.png)
   The INTERCOM | Developer Hub <img src="/images/partner_41411252388372.png" />window appears. Now, you must decide whether to create a new app or use an existing one.
...
7. Scroll to the **Permissions** section and ensure the permissions are enabled for your app.
   ![](/images/partner_41411462119828.png)

### Step 3: Create the Intercom API Data Enrichment Connector

<Info>
  **Information** Use the Intercom Contact Search API to get additional data, as shown in the following steps. For other tasks, you can use any of the APIs listed in the [Intercom API reference](https://developers.intercom.com/docs/references/rest-api/api.intercom.io/contacts/searchforcontacts).
</Info>

To create the Intercom Contact Search API, perform the following steps:

1. Create the MoEngage Webhook Data Connector. To do this, follow the steps through point 6 as detailed in the section Step 1: Create the MoEngage Webhook Data Connector.
2. In the **Request Body** section, paste the following JSON. This will search for a contact using their email or external ID.
   ...
   ```
3. Click **Next step** to proceed to the **Test response** tab to test the connection by providing sample values for your payload.
   ![](/images/partner_41456256508692.png)
   Note\*\*: Ensure Intercom is successfully connected to MoEngage before you use the connector in a live workflow.
...
8. Click **Save and set live**.
   ![](/images/partner_41456346752660.png)
...
1. Create the MoEngage Webhook Data Connector. To do this, follow the steps through point 7 as detailed in the section, Step 3: Create a Workflow to Trigger the Data Connector.
...
5. Click the **Save and Set**.
   ![](/images/partner_41456201382804.png)
...
3. On the **Series** page, click **\+ New Series** in the upper-right corner.
   ![](/images/partner_41454332176148.png)
   The **Choose a template** dialog box appears.
4. Click **Start from scratch**.
   ![](/images/partner_41454473940884.png)
   Now, you must set the entry rules.
...
6. Scroll to the end, in the **Actions to take** , click **WEBHOOK**.
   ![](/images/partner_41456101612564.png)
...
3. Select the check boxes for the specific audience you want to target.
   ![](/images/partner_41456101613972.png)
...
| --- | --- |
| **Method** | **POST** |
| **URL** | Enter the dedicated Webhook URL provided by MoEngage in the following format: [https://api-0X.moengage.com/v1/partner/intercom/](https://api-0X.moengage.com/v1/partner/intercom/{events/users}/?configName={config_name}){events/users}[/?configName=](https://api-0X.moengage.com/v1/partner/intercom/{events/users}/?configName={config_name}){config_name}; |
| **Webhook Headers** |  |
| Header(s) | Value |
| Content-Type | application/json |
