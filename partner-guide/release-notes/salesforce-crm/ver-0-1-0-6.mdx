...
| info |  **Information** Suppose a record was previously synced with MoEngage, and the mapped Salesforce field value for a mandatory MoEngage attribute (for example,`customer_id`or action) is later cleared (made blank). In that case, that record will no longer sync with MoEngage.  
