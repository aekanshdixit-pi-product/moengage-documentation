...
| 50 | Max concurrent jobs running on org before delaying batch execution. <br/>- Historic Sync Scheduler Time (min) | 

| 5 | |

 Delay before retrying historical sync when queue is full.  
Record Deletion Batch Size

| 2000 | Batch size for deletion of old successful records. <br/>- Record Deletion Interval (days) | 

| 7 | |

 Retention period for successful Sync Queue records.  
Sync Queue Batch Size

| 20 | Batch size for Real-Time processing. <br/>- Sync Queue Scheduler Time (min) | 

| 5 | |

 Delay before retrying Real-Time sync when the queue is full.  
