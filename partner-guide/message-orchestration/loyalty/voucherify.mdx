...
3. In the _Parameters_ tab:
   ...
   2. Add a _customer_ parameter and fill in the MoEngage field that should be used as the customer’s source\_id field in Voucherify. In this case, the customer’s email is used as source\_id. You can learn more about MoEngage users’ attributes [here](/user-guide/data/event-data/derived-events-and-attributes#tracked-standard-attribute-0-15).
...
2. Prepare the email content. Using the templating language Jinja you can insert e.g. customer name into the content and call the Content API endpoint created in step 2, parse its response, and display the desired information to the customer. You can find out more about using Jinja in MoEngage [in this article](/user-guide/campaigns-and-channels/getting-started/message-personalization/jinja-templating-language). kkk
