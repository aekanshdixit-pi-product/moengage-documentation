---
title: "Open Weather"
description: "Use Open Weather APIs through MoEngage Content APIs to enrich campaigns with real-time weather data and personalize messages based on local conditions."
keywords: ["open weather", "openweathermap", "weather personalization", "content api", "contextual marketing", "moengage integration"]
---
...
1. Navigate to **Settings** > **Advanced settings** > **Content API**. Click **+ Add content API** in the upper-right corner of the Content API screen.
   ...
   ![MoEngage Content API screen with the Add content API button highlighted](/images/partner_21979759864596.png)
2. Add the Current Weather API details.
   ...
   **Parameters**
   ...
   ![MoEngage parameter editor showing latitude and longitude mapped to user attributes](/images/partner_7428214057364.png)
3. Click **Save**.
4. The Open Weather API now appears in your Content APIs list.

   ![MoEngage Content APIs list with the new Open Weather API listed](/images/partner_7428241169300.png)
...
You can now use this Content API in your campaigns. For more information, see [Content APIs](/user-guide/campaigns-and-channels/getting-started/campaign-content/content-apis#step-2-using-content-apis-to-create-personalized-campaigns-0-3).

1. On step 2 of campaign creation, type `@`.
2. In the personalization pop-up, select the Open Weather Content API you configured in Step 1.

   ![MoEngage personalization editor with the Open Weather Content API being inserted](/images/partner_7428246362644.png)
3. Use the [MoEngage Templating Language](/user-guide/campaigns-and-channels/getting-started/message-personalization/jinja-templating-language#content-api-0-3) to parse the API response and define your campaign content based on local weather. For example:
...
Print the output in the template, or use [Personalized Previews](/user-guide/campaigns-and-channels/getting-started/message-personalization/personalized-preview) to preview. If the API response returns `Rain` as the weather description, the user receives the push message "It's raining. Get an umbrella".
