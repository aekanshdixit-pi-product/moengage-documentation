---
title: "Lokalise"
description: "Sync Lokalise translations into MoEngage Content APIs to deliver real-time, multi-language campaign content based on each user's language preference."
keywords: ["lokalise", "localization", "translation management", "content api", "multilingual campaigns", "moengage integration"]
---
...
## MoEngage and Lokalise

The MoEngage and Lokalise integration lets you deliver real-time, localized content across your campaigns without manual translation coordination. With this integration, you can:
...
- Streamline your translation workflow with Lokalise so your teams can focus on campaign strategy and customer engagement.
...
- **AWS S3 integration**: Leverage secure cloud storage for translation file management.
- **Template-based translation keys**: Create once in Lokalise and reuse across all campaigns.
- **Simplified localization workflow**: Eliminate manual translation file management and reduce time-to-market.
...
- **Real-time content updates**: Update product descriptions, promotional messages, or campaign copy in Lokalise and see the changes reflected immediately in MoEngage campaigns. There is no need to recreate or republish campaigns — translations update instantly.
...
- **Regulated market compliance**: Ensure compliance with regional marketing regulations by automatically serving legally required translated content tailored to each language and region.
...
<Info>
  **Prerequisites**
  ...
  - A MoEngage account with Content API access enabled.
  - A user attribute system set up in MoEngage to capture and track language preferences.
</Info>
...
Skip this step if you already have an existing Lokalise project. To create a new translation project:
...
4. Click **Create project**.
   ...
   ![Lokalise New Project dialog with project name, base language, target languages, and project type fields](/images/partner_31266962647188.png)

### Step 2: Define a user language attribute in MoEngage

To ensure MoEngage fetches the correct language variant from Lokalise, define a user attribute for language preferences first.
...
Send language data in your API calls with the property name `user_language`.
...
4. Complete the import process. For more information, see [Import User Data Using CSV](/user-guide/data/imports/file-based-imports/import-user-data-using-csv).
...
To define the content for your campaign, create translation keys in Lokalise:

1. Open your Lokalise project.
2. Click **Add Key**.
3. In the **Key name** box, enter the key.
4. In the **Base Language Value** box, enter the language value.
5. In the **Platforms** list, click **Web**.
6. Click **Save**.

   ![Lokalise Add Key dialog with key name, base value, and Web platform selected](/images/partner_31266916693524.png)
...
| --- | --- |
| `headline` | Campaign headline or main message. |
| `call_to_action` | Button text or CTA message. |
| `hero_image_alt` | Alternative text for hero images. |
| `product_title` | Name of a product or offer. |
| `discount_text` | Discount message or promotional text. |

#### Add translations

After creating the base language keys, provide translations for your target languages:
...
3. Click **Save**.
   ...
   ![Lokalise project editor with translations entered for multiple target languages](/images/partner_31266916707476.png)

<Warning>
  **Key naming conventions**

  To ensure compatibility with MoEngage, follow these rules:

  - Use underscores for spacing. For example, use `my_translation_key`.
  - Assign keys to the **Web** platform. All keys must be associated with **Web** to sync correctly.
  - Do not use periods. Periods (for example, `this.is.the.key`) cause integration errors.
...
To install the MoEngage app in Lokalise:

1. Open your Lokalise project and navigate to the **Apps** section.
2. Search for **MoEngage** and click **Install**.
3. After installation, the app generates a unique translation file URL in the following format:
...
4. Click **Refresh** in the MoEngage app to generate or update the latest translation files on the server.
   ...
   <Note>
     Click **Refresh** every time you add new keys or update translations in Lokalise to ensure MoEngage fetches the latest version.
   </Note>
5. Copy the generated URL. The URL stays the same even after you update your content.
6. (Optional) Test the URL by replacing the `{{${language}}}` placeholder with an actual language code (for example, `en` or `es`) and opening it in your browser. You should see a JSON file containing your translation keys and values.
...
To configure the Content API in MoEngage:
...
3. Enter the following details:
   ...
   2. **Method**: Select `GET`.
   3. **URL**: Paste the translation file URL you copied from Lokalise. For example:
   ...
   4. **Map the attribute**: Replace the `{{${language}}}` part of the URL with your MoEngage user-language attribute.
      ...
      <Note>
        The attribute name (for example, `user_language`) must exactly match the one you defined in Step 2.
      </Note>
4. Click **Test**, enter a language code (for example, `es` for Spanish or `fr` for French), and verify that the API returns a JSON response with your translation keys.
5. Click **Save** to complete the setup.

   ![MoEngage Content API configuration with the Lokalise URL and user language attribute mapped](/images/partner_31453674918036.png)

### Step 6: Use Content API in campaigns

To use the Content API in campaigns:

1. In the MoEngage UI, on step 2 (Campaign Content) of campaign creation, type `@` in any text field (headline, body, or CTA). For more information, see [Content APIs](/user-guide/campaigns-and-channels/getting-started/campaign-content/content-apis).
2. From the dropdown menu, select the Lokalise Content API you configured in the previous step.
...
You can use the [MoEngage Templating Language](/user-guide/campaigns-and-channels/getting-started/message-personalization/jinja-templating-language#content-api-0-3) to manipulate how translations are displayed or to create fallback logic:
