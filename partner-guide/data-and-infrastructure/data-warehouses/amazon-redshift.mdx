...
description: "Sync user attributes and events from Amazon Redshift directly to MoEngage on a recurring schedule to power campaigns and segmentation."
keywords: ["Amazon Redshift", "data warehouse", "user sync", "event sync", "MoEngage integration", "AWS"]
---
...
From the Redshift Console, open your Redshift cluster.
  ...
  3. From the popup modal, select the Role that you just created in [Step 5] (https://moengage-partner-guide.mintlify.app/partner-guide/data-and-infrastructure/data-warehouses/amazon-redshift#step-5-create-a-new-redshift-iam-role).
...
| Host Name | On your AWS Console, navigate to your Redshift cluster. Under **General information**, the hostname is listed as your Redshift endpoint. Paste the part before the colon `:`. For example, if the Endpoint is `redshift-cluster-1.xxxxxxxzgxxx.us-east-1.redshift.amazonaws.com:5439/dev` then your Host Name will be `redshift-cluster-1.xxxxxxxzgxxx.us-east-1.redshift.amazonaws.com`. | Format should be `[name].[id].[region].redshift.amazonaws.com` |
| Port | Enter your port number. On your AWS Console, navigate to your Redshift cluster. Under the **Properties** tab, find your port number and database name in the **Database configurations**. By default, the Redshift port number is 5439. | `5439` |
| Database | The name of your Redshift database. | `moengage_exports` |
| Username | Enter the username you want MoEngage to connect as. This can either be an existing user, or the one you created in [Step 2](https://moengage-partner-guide.mintlify.app/partner-guide/data-and-infrastructure/data-warehouses/amazon-redshift#step-2-create-a-new-database-user-for-moengage). | — |
| Password | Enter the password of the database user. | — |
| IAM Role ARN | Enter the ARN of the IAM Role you created in [Step 5] (https://moengage-partner-guide.mintlify.app/partner-guide/data-and-infrastructure/data-warehouses/amazon-redshift#step-5-create-a-new-redshift-iam-role). To find the ARN, on your AWS Console, go to IAM Console. In the side menu, under Access Management, click on **Roles**. Select the IAM Role you created in . Under the **Summary** section, you will find the ARN listed. Copy and paste it into the input field. | Format should be `arn:aws:iam::[aws-account-id]:role/[role-name]` |
...
![Redshift endpoint location](/images/partner_12091527172244.png)

![Port and database configuration](/images/partner_12091625026324.png)

![IAM Role ARN location](/images/partner_12091830424212.png)
