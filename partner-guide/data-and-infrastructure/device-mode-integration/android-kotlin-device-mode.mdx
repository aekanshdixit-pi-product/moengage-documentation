...
Get Workspace ID (earlier App ID) from the Settings Page Dashboard --> Settings --> Account --> General and initialize the MoEngage SDK in the _Application_ class's _onCreate()_  
...
We recommend you add the callbacks in the _**onCreate()**_ of the Application class since these callbacks can be triggered even when the application is in the background.
