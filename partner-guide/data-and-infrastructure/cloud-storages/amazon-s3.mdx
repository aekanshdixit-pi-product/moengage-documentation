...
<AccordionGroup>
  <Accordion title="Cross-account role (Recommended) — supports both imports and exports">
    ...
    - **MoEngage AWS Account ID**: The MoEngage account your role trusts (`582272135730`).
    - **MoEngage External ID**: A value unique to your workspace, in the format `moe-external-xxxxxxxxxx` (for example, `moe-external-f82a6992-9c75-5d4c-f985-a0c3b8f3bcf2`).

    <Info>
      **Where to find these values**

      In MoEngage, go to **Amazon S3** \> **Integrations** \> **Add Connection**. The **MoEngage AWS Account ID** and your workspace's **MoEngage External ID** appear on the **Add Connection** form (see [Step 3: Connect Amazon S3 on the App Marketplace](#step-3-connect-amazon-s3-on-the-app-marketplace)). Copy them before you configure AWS.
    </Info>

    ![Add Connection form highlighting the MoEngage AWS Account ID, MoEngage External ID, and Your AWS role ARN fields](/images/s3-add-connection-form.png)
    ...
    **Create the cross-account role**
    ...
    4. In the **Custom trust policy** editor, paste the trust policy shown below (replace the placeholders first), and then click **Next**.
    ...
    **Trust policy**
    ...
    The trust policy defines who may assume your role. Paste the following into the **Custom trust policy** editor:
      ...
      "Statement": [
        {
          ...
          "Effect": "Allow",
          "Principal": {
            "AWS": "arn:aws:iam::582272135730:root"
          },
          "Action": "sts:AssumeRole",
          "Condition": {
            "StringEquals": {
              "sts:ExternalId": "moe-external-xxxxxxxxxx"
            }
          }
        }
      ]
  ...
  <Accordion title="IAM policy (Exports) — supports exports only">
    ...
    2. Add the following statement. Replace `<INSERTPOLICYID>` and `<INSERTSID>` with values of your choice (any unique identifiers), and `acme-moengage-data` with your bucket name. Keep the MoEngage **Principal** ARN exactly as shown.
      ...
      "Statement": [
        {
          ...
          "Action": "s3:PutObject",
          "Resource": "arn:aws:s3:::acme-moengage-data/*"
        }
      ]
    }
    ```
...
In each example, the bucket is named `acme-moengage-data` and files live under the `moengage/` folder. Replace these with your own bucket name and folder path.

<Note>
  **Bucket ARN vs. object ARN**

  Bucket-level actions like `ListBucket` use the bucket ARN (`arn:aws:s3:::acme-moengage-data`). Object-level actions like `GetObject` and `PutObject` use the object ARN (`arn:aws:s3:::acme-moengage-data/moengage/*`).
</Note>
      ...
      "Statement": [
        {
          ...
          "Effect": "Allow",
          "Action": ["s3:GetObject"],
          "Resource": "arn:aws:s3:::acme-moengage-data/moengage/*"
        },
        {
