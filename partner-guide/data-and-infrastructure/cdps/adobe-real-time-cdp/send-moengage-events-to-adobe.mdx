...
description: "Stream MoEngage campaign events and user actions to Adobe Experience Platform via MoEngage Streams to build unified customer profiles."
keywords: ["Adobe Experience Platform", "AEP", "MoEngage Streams", "event streaming", "unified profile", "MoEngage integration"]
---
...
1. Click **Create identity namespace**.
...
1. Create your custom XDM ExperienceEvent schema with your desired field structure.
2. Ensure that you include the following required fields.
   - `_id` (required for deduplication)
   - `timestamp` (required)
   - `identityMap` (required for identity resolution)
   - `eventType` (recommended)
3. Add custom fields for MoEngage data under your tenant namespace.
4. To enable the schema for profiles, turn the **Profile** toggle on.
...
![Stepbndhu4](/images/stepbndhu4.png)
...
![Step5bindhu](/images/step5bindhu.png)
...
![](/guide-media/01KCNZQ7WVMH6NQ32J26T0R2N4)
...
<Tabs>
  ...
  <Tab title="Manual Mapping for Local Testing">
    ...
    2. Key mappings to configure:
       - **id** → `_id`
       - **event_uuid** → `eventMergeId`
       - **Use formula**: `dformat((time * 1000), "yyyy-MM-dd'T'HH:mm:ssX")` → `timestamp`
       - **event_code** → `eventType` (with decode formula for XDM event types)
...
```text Endpoint URL
[https://dcs.adobedc.net/collection/](https://dcs.adobedc.net/collection/){UNIQUE_COLLECTION_ID}
```
