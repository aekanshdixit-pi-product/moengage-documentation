...
| info |  Note Ensure to replace Your_Workspace_ID with the actual Workspace Id from MoEngage Dashboard -> Settings -> Account -> General Settings For dataCenter, please contact our support team to know more.

| --- | --- |  
  
| info |  Note To redirect data to test environment, append '_DEBUG' to appId. For example, if your appId is   
...
| warning |  Important If the user log in and log out is not handled correctly, user data may get corrupted. Refer [this section](https://developers.moengage.com/hc/en-us/articles/360061114832-Web-SDK-User-Attributes-Tracking) for more details
...
| warning |  Important After performing the above Moengage logout event, do NOT send "identifiers" with any further tracking code. Because the user had logged-out and further attribute/event tracking must not be associated with this user.
