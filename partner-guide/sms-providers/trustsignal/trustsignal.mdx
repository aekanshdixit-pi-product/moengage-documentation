...
| API URL | Select the **POST** option. POST API: * Endpoint: [https://sms.trustsignal.io/v1/sms/moengage](https://sms.trustsignal.io/v1/sms/moengage) * Enter the JSON payload structure supported by the POST endpoint. |
| Parameter Details | Key value pair / JSON parameter details: * apikey=`******` [Trustsignal user account APIKEY] * sender_id= [DLT approved Header] * to=`Moesms_destination` [Mobile number on which SMS to be sent with 91] * message=`Moesms_message` [DLT approved message text] * template_id=`Moesms_dltTemplateId` [DLT Template ID for message text] * route=[transcational/promotional] |
