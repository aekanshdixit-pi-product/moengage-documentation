...
<Info>
  **Prerequisites** Ensure you have access to your Infobip Dashboard and have the API key for your Infobip account.
</Info>
...
| **Body Type** | Choose the Body Type as **JSON** and add the following details:<br /><br /><code>{'{'}<br />    "messages":[<br />    {'{'}<br />        "destinations":[{'{'}"to":"Moesms_destination"{'}'}],<br />        "from":"SENDER ID",<br />        "text":"Moesms_message",<br />        "intermediateReport":"True",<br />        "regional": {'{'}<br />            "indiaDlt": {'{'}<br />                "principalEntityId": "XXXXXXXXXX",<br />                "contentTemplateId": "Moesms_dltTemplateId"<br />            {'}'}<br />        {'}'},<br />        "notifyContentType":"application/json",<br />        "notifyUrl":"YourMoEngageAccountURL"<br />     {'}'}]<br />{'}'}</code>  <br /><br />Change the values of Sender\_ID and YourMoEngageAccountURL based on your Infobip and MoEngage accounts. |
...
![Infobip\_NewUI\_PG\_DT.png](/images/partner_21970648326804.png)
