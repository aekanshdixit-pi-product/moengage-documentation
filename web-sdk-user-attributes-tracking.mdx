...
<Tabs>
  <Tab title="JavaScript">
    ...
    Moengage.add_user_attribute('someCustAttr', { 
      'key1': { 
        'subKey1': 'someVal1', 
        'subKey2': true, 
        'subKey3': 125676, 
...
<Warning>
  **Warning** If the user [log in](#log-in) and [log out](#log-out) is not handled correctly, user data may get corrupted.
</Warning>
...
<Warning>
  **Critical - Very Important Integration Guideline** Never use both the login methods - `identifyUser` and `add_unique_user_id` (login method of SDK versions below 2.52.2) in your project. Use only either one of the methods. Using both the methods can lead to inconsistent user profile creation and merging in your MoEngage account.
</Warning>
...
<Warning>
  **Critical** `identifyUser` and `destroy_session` methods should be called in proper order. So make sure, you track other attributes after these methods are executed completely. Since these return promise, you can track data like this:
...
4. For more information on supported data types and data tracking policies, please refer to [Data Tracking Policies](https://help.moengage.com/hc/en-us/articles/360037314991-Data-Tracking-Policies?_gl=1%2A1nx53wd%2A_ga%2AMTU4NTM0MzI0NS4xNzI3MTcwMzYy%2A_ga_SEBHW7YTZ7%2AczE3NzE4MjEwNDEkbzE1NiRnMSR0MTc3MTgyOTk3NCRqNjAkbDAkaDA.#h_01H9QNZ61VYYESY6FRMTQXVWX0).
...
For more information about the default attributes collected by MoEngage SDK, refer to [Web SDK Data Collection](https://help.moengage.com/hc/en-us/articles/360042954831-Default-Web-SDK?_gl=1%2A1q0t46u%2A_ga%2AMTU4NTM0MzI0NS4xNzI3MTcwMzYy%2A_ga_SEBHW7YTZ7%2AczE3NzE4MjEwNDEkbzE1NiRnMSR0MTc3MTgzMDA0MiRqNjAkbDAkaDA.).
