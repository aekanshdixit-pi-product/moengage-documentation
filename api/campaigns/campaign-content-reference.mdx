...
- For campaigns in `ACTIVE` state, the following fields **cannot** be edited: `trigger_condition`, `segmentation_details`, `conversion_goal_details`, the scheduling type, and the scheduling start date. `campaign_content` and `basic_details.platforms` **can** be edited. The full per-state matrix is at [Update Campaign](/api/patch-draft-campaign-v5).
...
- [Update Campaign](/api/patch-draft-campaign-v5) — per-state edit restrictions, publish action.
- [Update Campaign Status](/api/change-campaign-status-v5) — `STOP`, `PAUSE`, `RESUME` transitions.
- [Validate Campaign](/api/validate-draft-campaign-v5) — pre-publish validation check.
