...
This reference covers the request-body components that define campaign content for Push and Email campaigns: `basic_details` (identifying metadata, platform targeting, and platform-specific flags) and `campaign_content` (the message payload per channel, platform, and template type, with locale and A/B variations). Used by [Create Campaign](/api/v5/create-campaigns/create-campaign) and [Update Campaign](/api/v5/update-campaigns/update-campaign).
...
Optional rich media: `rich_media_type` (`Image`, `Video`, or `GIF` — title case) with `rich_media_value` (the URL). `input_gif_url` is supported on `BASIC` and `STYLIZED_BASIC`.
...
- For campaigns in `ACTIVE` state, the following fields **cannot** be edited: `trigger_condition`, `segmentation_details`, `conversion_goal_details`, the scheduling type, and the scheduling start date. `campaign_content` and `basic_details.platforms` **can** be edited. The full per-state matrix is at [Update Campaign](/api/v5/update-campaigns/update-campaign).
...
- [Create Campaign](/api/v5/create-campaigns/create-campaign) — required fields, happy-path cURLs, error responses.
- [Update Campaign](/api/v5/update-campaigns/update-campaign) — per-state edit restrictions, publish action.
- [Validate Campaign](/api/v5/create-campaigns/validate-campaign) — pre-publish validation check.
