...
* [Create Campaign](/api/create-draft-campaign-v5): Initializes a new Push or Email campaign in a `DRAFT` state using minimal required fields.
* [Get Campaign](/api/get-single-campaign-v5): Retrieves a single campaign in its full deparsed form, displaying its current state and configuration.
* [Update Campaign](/api/patch-draft-campaign-v5): Updates specific components of an existing draft or triggers the formal `PUBLISH` action to launch the campaign.
* [Validate Campaign](/api/validate-draft-campaign-v5): Safely runs a full publish-time validation check without modifying the draft or changing its state.
* [Update Campaign Status](/api/change-campaign-status-v5): Applies lifecycle transitions (STOP, PAUSE, RESUME) to campaigns that have already been published.
* [Search Campaigns](/api/search-campaigns-v5): Searches for campaigns using granular filters, with the ability to explicitly include or exclude campaigns in draft state.
* [Get Campaign Meta](/api/get-campaign-meta-v5): Fetches lightweight metadata, including daily cached reachability estimates for scheduled campaigns.
* [Test Campaign](/api/test-campaign-v5): Sends a test push or email to up to 10 users. Supports **inline mode** (pass `channel` and `campaign_content` directly) and **draft mode** (pass `draft_id` to load content from a saved draft). Draft mode is new in V5.
* [Get Child Campaigns](/api/get-child-campaigns-v5): Retrieves the execution history of a Periodic or Business Event-triggered campaign, returning each child campaign's ID, send time, and status.
* [Personalized Preview](/api/preview-personalized-content-v5): Returns a resolved preview of personalized campaign content for a specific user without sending a message.
* [Update Global Control Group](/api/update-global-control-group-v5): Adds or removes users from the Global Control Group in bulk via a CSV file.
...
<AccordionGroup>
  ...
  <Accordion title="How do I find the campaign ID to use in the update request?">
    The `campaign_id` is returned in the `data.id` field of the Create Campaign response. You can also retrieve it using `GET /v5/campaigns/{campaign_id}` or by using the [Search Campaigns](/api/search-campaigns-v5) endpoint to look up campaigns by name, status, channel, or delivery type.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="How do I know which fields are required for a specific Push template type?">
    ...
    For the `Custom` template type, `custom_template_id` is required. For all other types, refer to the `campaign_content` schema in the [Create Campaign](/api/create-draft-campaign-v5) reference for the full list of required and optional fields per template type.
  </Accordion>
</AccordionGroup>
