...
- [Delete User](/api/user/delete-users-in-bulk): Deletes users in MoEngage.
- [Create Event](/api/event/create-event): Tracks the actions of a user.
...
- [Bulk Import](/api/bulk/bulk-import-users-and-events): Sends multiple user and event requests in batch to MoEngage, using a single API request.
- [Install Tracking](/api/tracking/track-app-install): Tracks the install attribution data in MoEngage.
- [Test connection](/api/utilities/test-api-connection): Validates if the entered endpoint details are valid.
- [MoEngage Streams](/api/webhooks/moengage-streams-webhook): Forwards events available in MoEngage to a predefined API endpoint.
