...
Aggregation facilitates growth teams to aggregate an [event attribute](/user-guide/data/event-data/derived-events-and-attributes) by using operations such as sum, average, min, max, and median. The aggregation operator on an event attribute combined with the [numeric filters](/user-guide/segment/advanced-concepts/filters-in-segmentation) helps users write sophisticated queries and extract deeper customer segments.
