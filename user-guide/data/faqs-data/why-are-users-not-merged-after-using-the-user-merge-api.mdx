...
1. *Update the payload*. Update the following payload with the ID that must be merged and retained.
   ...
   ```
2. *Use the correct identifier.* The UID is the unique identifier for a user, which MoEngage stores in the [ID attribute of the user profile](/user-guide/data/user-data/user-profile). Ensure that only the UID/ID attribute in the user profile is used to merge users. <img alt="user profile id.png" src="/images/moengage_896d2c.png" />   
   <Info>
