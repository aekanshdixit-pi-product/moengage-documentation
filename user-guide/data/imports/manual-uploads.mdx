...
<Tabs>
  <Tab title="Users">
    ...
    **Entry point:** **Segment** > **User Imports** > **Import Users**.
    ...
    - **Configure the identifier**: For registered users, mark one column as **ID**. If you have [Unified Identity](/user-guide/data/user-data/unified-identity-identity-resolution) enabled, you will have an option to map the additional identifiers as well. For anonymous users, mark a column as **Anonymous ID**.
    ...
    Select **Update existing users only** to bulk-update attributes on users MoEngage already has. MoEngage creates no new users from that upload, and a row whose identifier is not already in MoEngage is ignored. Leave it off to let the upload create users as well as update them.
...
- Multi-line support in CSV files is disabled by default. Please get in touch with support if you need this feature. Newline formats other than `\n`, such as `Ctrl+M` and `\r`, are not supported. Enabling multi-line support can negatively impact the processing speed of your import.
- JSON files.
