...
- **User imports (Registered)**: map your identifier column to **User ID**. If you have [Unified Identity](/user-guide/data/user-data/unified-identity-identity-resolution) enabled, you will have an option to map the additional identifiers as well.
...
For user imports, select **Update existing users only** under **Import Behaviour** to bulk-update attributes on users MoEngage already has. MoEngage creates no new users from that import, and a row whose identifier is not already in MoEngage is ignored.
