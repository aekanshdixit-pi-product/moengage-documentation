...
<Accordion title="Deep dive: entity behavior, timezone pitfalls, and a worked example">
  ...
  **Events** — two separate mappable columns:
  ...
  ### Worked Example (Hourly Import)

  Suppose you create an hourly import at `10:07 UTC`. The first `next_run_time` fast-forwards to the next hour boundary, `11:00 UTC`. The **historic run** has no lower bound and catches every matching row up to `11:00`.

  **Run 1 (`11:00`–`12:00`)** — the source table has one new row:

  | user_id | updated_at (UTC) | Imported in |
  | --- | --- | --- |
  | u_101 | 11:30 | Run 1 |

  The `11:30` row falls inside `[11:00, 12:00)`, so it is caught cleanly in Run 1.

  **Run 2 (`12:00`–`13:00`)** — two more rows have landed:

  | user_id | updated_at (UTC) | Imported in |
  | --- | --- | --- |
  | u_102 | 12:00:00 | Run 2 |
  | u_103 | 09:15 *(written to the table at 12:45)* | never |

  `u_102` at exactly `12:00:00` was excluded from Run 1 (`< 12:00` fails there) and is caught in Run 2 (`>= 12:00` passes) — no gap, no double-count. `u_103` was physically written at `12:45` wall-clock but carries a reference timestamp of `09:15` — already `< 11:00`, so it missed Run 1's window and is `< last_run_time` for every future window.

  **Run 3 (`13:00`–`14:00`)** — the source table still contains `u_103`:

  | user_id | updated_at (UTC) | Imported in |
  | --- | --- | --- |
  | u_103 | 09:15 | never |

  `u_103` does not appear in Run 3 — or any later run — because its `09:15` reference timestamp is earlier than every future `last_run_time`. The row is permanently missed by periodic sync, even though it was added to the table only moments before Run 3.
