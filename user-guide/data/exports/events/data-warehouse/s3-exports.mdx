...
Navigate to the **Policies** tab in the AWS IAM navigation bar and select **Get Started**, then **Create Policy**. Select **Create Your Own Policy** to restrict MoEngage access to only the specific bucket you define.

![Create an access policy in the AWS IAM console](/images/moengage_caa65f.png)

In the **Policy Document** section, enter the following JSON. Replace **INSERTSID** with your SID, **INSERTPOLICYID** with your policy ID, and **INSERTBUCKETNAME** with your bucket name.
...
## Step 3: Enable S3 exports on the App Marketplace

Set up an S3 connection in the MoEngage App Marketplace, and then add an export on top of that connection.

1. On the navigation menu in the MoEngage UI, click **App marketplace**.
2. In the search bar, type **Amazon S3**, and then click the **Amazon S3** tile.
3. Click the **Integrate** tab, and then click **+ Add connection**. <img src="/images/moengage_a20cd3.png" alt="Integrate tab with the Add connection option" />
4. Configure your S3 bucket connection by using the following parameters:

   | Field | Description | Example |
   | --- | --- | --- |
   | **Connection name** | Type a unique and identifiable name for this connection. | My S3 Export |
   | **Bucket name** | Type the name of your S3 bucket.  <br />**How do I find the bucket name?**<br />In your AWS console, go to **S3** and select your bucket from the list. The bucket name appears at the top. <img src="/images/moengage_ca9a6b.png" alt="Bucket name shown at the top of the S3 bucket list" /><br />You can also find the name on the bucket **Properties** tab. In your **S3 ARN**, the portion after `arn:aws:s3:::` is the bucket name: <img src="/images/moengage_ae5dee.png" alt="Bucket name in the S3 ARN on the Properties tab" /> | `test-bucket` |
   | **Bucket region** | Select the AWS region of your S3 bucket. | `Asia Pacific (Mumbai)` (ap-south-1) |

5. Click **Test connection** to validate, and then click **Save connection**. The saved connection appears as a card showing the host, the user who last updated it, and the last updated timestamp.
6. Under the **Setup exports to S3** section, click **+ Add export** to open the **Export details** panel.
7. Configure the export by using the following parameters:

   | Field | Required | Description |
   | --- | --- | --- |
   | **Export name** | Yes | Type a unique and identifiable name for the export (for example, `File-exports2`). UI placeholder: *Enter Export name*. |
   | **S3 connection** | Yes | Select a saved connection from the dropdown list. When you select a connection that uses an IAM-policy auth method, MoEngage shows the helper text *S3 exports only: This method requires you to configure an IAM policy in your AWS account. MoEngage will use this established permission to deliver files to your S3 bucket during data exports. Read more*. |
   | **Folder path** | Yes | Type the path to the target folder where MoEngage must export your files. Do not begin or end the string with a slash. UI placeholder: *Enter Folder path*.  <br />Select the **Export all files in one folder** check box to write every export to a single folder instead of date-partitioned subfolders. The resolved file path is rendered live below the field so that you can confirm the destination before saving. The format depends on whether you selected the check box: <ul><li>**Single folder (check box selected):** `s3://<your-bucket-name>/[<folder_path>]/<connection_name>/event_exports_<file_tag>.json.gz`</li><li>**Date-partitioned (check box cleared):** `s3://<your-bucket-name>/[<folder_path>_]<connection_name>/export_day=YYYY-MM-DD/event_exports_<UNIQUE_ID>.json.gz`</li></ul>**How do I find the folder path?**<br />From the list of objects in your **S3** bucket, open the folder where you want to dump the files. On the **Properties** tab, locate the folder's **S3 URI**. Your folder path is the part after `s3://[your-bucket-name]/`, excluding the trailing slash. |
...
8. Click **Test**.  <br />MoEngage attempts to write a temporary placeholder file to verify your S3 bucket configuration, folder path, and write permissions.
9. When the test passes, click **Save**. MoEngage displays a confirmation popup that calls out the locked frequency. Review the message and click **Save** again to activate the export.
10. To discard the configuration without saving, click **Cancel**. <img src="/images/s3-exports-config-panel.png" alt="Export details panel with Test, Save, and Cancel options" />
...
- **Sample folder path**:
  ...
  - *Single folder*: `s3://<your-bucket-name>/[<folder_path>]/<connection_name>/event_exports_<file_tag>.json.gz`

  MoEngage generates the `<UNIQUE_ID>` or `<file_tag>` as a unique identifier for each export file.
