...
sidebarTitle: "Use PGP Encryption in MoEngage"
---
...
Enter the private key in the **Decryption Key** section on the MoEngage UI, and then enter the passphrase in the **Key Passphrase** section. For more information, refer to the [Adding your S3 Credentials (S3 Imports)](https://help.moengage.com/hc/en-us/articles/10170540879380-S3-Imports#h_01HDQXTY70NCXXFGJQC5DSQS7G) and [Adding your S3 Credentials (SFTP Imports)](https://help.moengage.com/hc/en-us/articles/12241329478676-SFTP-Imports#h_01HDR27GJTY7X6PYNQW3F1PCAH)  sections.
...
key\_data = ""
import\_result = gpg.import\_keys(key\_data)
gpg.trust\_keys(import\_result.fingerprints, "TRUST\_ULTIMATE")
#
encrypted\_file\_path = "/path/to/encrypted/file"
with open(encrypted\_file\_path, 'rb') as file:
status=gpg.decrypt\_file(file, passphrase="", output="/path/where we want to store the decrypted file")
print(status.ok)
print(status.stderr)
```
...
```
if status.signature\_id: # if signature verified then it has value else None
...
1. [S3 Imports](https://help.moengage.com/hc/en-us/articles/10170540879380-S3-Imports)
2. [SFTP Imports](https://help.moengage.com/hc/en-us/articles/12241329478676-SFTP-Imports)