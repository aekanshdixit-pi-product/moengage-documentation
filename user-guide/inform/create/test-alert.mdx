...
The channel level payload's recipient field is looked at first and based on the value provided for the recipient there; the Alert is sent to the mobile number/email ID or push token specified. In case the recipient details are not available, the _user\_id_has to be passed in the request, failing which, the request will be dropped. If the _user\_id_ (the ID field in the [User Profile](/user-guide/data/user-data/user-profile))is available in the payload and the recipient details(mobile number/email ID/active push token) for the specified channel are available in the user profile, then the Alert is sent.
