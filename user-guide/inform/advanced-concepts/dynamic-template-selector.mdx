...
7. **\+ Nested filter:** Click this option to add sub-conditions beneath a primary attribute line, allowing you to build complex dependencies (e.g., Check if `User attribute` is True **AND** a secondary sub-variable matches a specific value).
8. **\+ Filter:** Click this button to append an entirely new operational condition row within the same rule card, using logical operators (like AND/OR) to join multiple attribute validation lines together.
9. **Add rule:** Click this button to generate a brand new, separate rule card block. This allows you to chain multiple distinct variations in priority order within the same overarching Rule Set.
...
1. Navigate to **Step 2: Create** of Inform alert creation journey.
