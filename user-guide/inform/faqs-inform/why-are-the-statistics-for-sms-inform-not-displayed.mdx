...
<Accordion title="How can we segment the number of messages sent through Inform?">
  ...
  2. Select **User behavior** and choose the relevant channel event (e.g., `Email Sent`, `SMS Sent`, or `Push Notification Sent`).
  ...
  \*\*Note: \*\*If you want to track the reach of one specific alert rather than all of them, follow the exact steps above, but change the operator from **exists** to **is**, and select your specific  Alert ID into the value field (e.g., _Alert ID is 69c63d7218dfa5e41e2f1c73_).
</Accordion>
