...
Each Custom Agent is designed for a specific, single-purpose task. The scope of an agent's capabilities is strictly defined by the tools and permissions you assign to it during configuration. Depending on these assigned tools, agents can perform various automated actions. For more information, refer to Available Tools. 

## Navigating the Custom Agents Dashboard

When you navigate to **AI Agents** > **Custom Agents** from the left menu, you land on the Custom Agents dashboard. This centralized page allows you to search, manage, and execute all Custom Agents available within your MoEngage environment.
