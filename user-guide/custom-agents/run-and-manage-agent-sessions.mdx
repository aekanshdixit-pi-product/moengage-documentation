...
* **From the Agent Detail Page:** Open the specific agent and click **Run** in the top-right corner.
* **From the Configuration Builder:** While creating a Draft or editing a Live agent, click **Test agent** to start a trial run. This session will be explicitly tagged as **Test** in the agent's history.
...
<Info>
  **Info:** Agents rely entirely on your instructions and the context you provide. Supplying necessary IDs, dates, or reference briefs in your very first message prevents unnecessary back-and-forth and improves output accuracy.
</Info>
...
* **No Auto-Send Capabilities:** Even with write tools enabled, an agent can only create, edit, or pause campaigns. It cannot finalize, publish, or send them. Final deployment must always be executed manually through the standard MoEngage campaign builder.
...
* **Immutable History:** Sessions cannot be deleted. They remain permanently in the agent's history. To prevent accumulating sessions on an obsolete agent, you must archive the agent entirely, which removes its history from gallery access.
