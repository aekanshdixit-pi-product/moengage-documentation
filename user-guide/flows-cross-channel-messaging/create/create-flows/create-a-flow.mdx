...
3. From the resulting pop up, use one of the following options to create a flow:
   - Using Merlin AI Flows Assist or
...
<Tabs>
  ...
  <Tab title="Reviewer">
    To review the flow, perform the following steps:
      ...
      2. Click the flow you want to open. <img src="/images/moengage_faf4b3.png" />
   You must review the flow by verifying the configurations of every stage. After the review is complete, and if there are no corrections required for the flow, perform the following steps:
      ...
      3. Click **Approve & publish**. <img src="/images/moengage_95d106.png" /> After you publish, the flow becomes **Active** or **Scheduled** based on the flow (for example, **On event trigger** or **At fixed time**). The creator and the other approvers receive an email notification informing them that the flow is published.
