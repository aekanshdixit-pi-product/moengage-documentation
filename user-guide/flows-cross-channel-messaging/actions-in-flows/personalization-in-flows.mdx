...
sidebarTitle: "Personalization in Flows"
---
...
[MoEngage Flows](https://help.moengage.com/hc/en-us/articles/26100424636436-Create-a-Flow) are a series of cross-channel campaigns (Email, Push, SMS, and so on) you can send to users based on their actions or inactions on your app or website.
...
| --- | --- |
| `{{EventAttribute['MOE\_TRIGGER\_EVENT.Add to Cart. Product Id']}}` | This will resolve to 1234 because it uses the data from the flow's trigger event. |
...
| `{{EventAttribute['Add to Cart. Product Company']|default('MOE\_NOT\_SEND')}}` | MoEngage does not send the campaign with this Jinja, and displays a "Personalization error" because the latest "Add to Cart" event instance (10:30 AM) does not have the 'Product Company' attribute. |