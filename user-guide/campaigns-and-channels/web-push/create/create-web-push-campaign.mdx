...
|  |  |
| --- | --- |
| - **Ignore frequency capping** - Enable this toggle to ignore the frequency capping settings. For more information, refer to [Frequency Capping](/user-guide/settings/channels/delivery-controls/frequency-capping). - **Ignore global minimum delay** - Enable this toggle to ignore the settings for the minimum delay set for Push notifications. - **Request Limit** - We support a maximum throughput of 250K requests per minute. If multiple campaigns are running in parallel, then we might not be able to support throughput for all the campaigns and campaigns might be delayed | <img src="/images/moengage_8893d5.png" /> |
