...
Tests can be used to test a variable against a common expression. To test a variable or expression, add **_is_** and the name of the test after the variable.

For example, to find out if a variable is defined, you can do "_name is defined"_, which will then return true or false depending on whether the name is defined in the current context of the template.
...
For more information, refer to [Jinja Pallets](https://jinja.palletsprojects.com/en/3.1.x/templates/).

## Comments

To comment out part of a line in a template, use the comment syntax, which is by default set to _`{# ... #}`_. This is useful to comment out parts of the template for debugging or to add information for other template designers or yourself:
...
It is sometimes desirable (even necessary) to have MoEngage ignore parts it would otherwise handle as variables or blocks. For example, if, with the default syntax, you want to use _`{{`_ as a raw string in a template and not to start a variable, you must use a trick.
...
_you_ **must** escape it unless the variable contains well-formed and trusted HTML. Escaping works by _piping_ the variable through the
...
{% for interest in UserAttribute.interests %}- {{ interest|e }}
{% endfor %}
{% endif %}
```
...
Brand is Puma!
{% else %}
{% MOE\_NOT\_SEND("Brand name doesn't exist for user") %}
{% endif %}
```
...
```ruby
{% set has\_first\_name = UserAttribute['First Name] and UserAttribute['First Name']|length 0 %}
{% set has\_last\_name = UserAttribute['Last Name'] and UserAttribute['Last Name']|length 0 %}
{% if has\_first\_name and has\_last\_name %}
Hello {{UserAttribute['u\_n']}} {{UserAttribute['u\_ln']}}!
{% elif has\_first\_name and not has\_last\_name %}
{% MOE\_NOT\_SEND("Last Name doesn't exist for user") %}
{% elif not has\_first\_name and has\_last\_name %}
{% MOE\_NOT\_SEND("First Name doesn't exist for user") %}
{% else %}
{% MOE\_NOT\_SEND("Both First Name and Last Name doesn't exist for user") %}
{% endif %}
```
...
- Scenario 1: If the last name is missing, the _Last Name doesn't exist for user_error appears.
...
<Info>
  MoEngage currently supports all other standard functions of Jinja. If you want to learn more about standard functions in Jinja, refer to [List of Global Functions](https://jinja.palletsprojects.com/en/3.1.x/templates/#list-of-global-functions).
</Info>
...
\> _Usage_`{{'14/10/2020'|dateFormatter('%m/%d/%Y')}}`
...
\> _Usage (For a MM/DD/YYYY in EST Timezone)_`{{'%m/%d/%Y'|today('EST')}}`
...
\>\
\> Usage`{{ "2012-01-19 17:21:00 CST" |dateTimeFormatter(toFormat='%Y-%m-%d %H:%M:%S %p')}}`
...
\>\
\> _Usage_`{{ "2012-01-19 17:21:00 CST" |dateTimeFormatter(tzOffset=-330)}}`\
...
BEFORE: {{ item }}
{% if item == continue\_condition %}
SKIPPING (simulating continue)
{% continue %}
{% endif %}
{% if item == stop\_condition %}
BREAKING
{% break %}
{% endif %}
AFTER: {{ item }}
...
"continue\_condition": "continue\_value",
"stop\_condition": "stop\_value"
}
```
...
\> _Usage (where 6ABC89W3XY9W is secret key)_`{{UserEmail|convertToSHA256('6ABC89P3FXYW')}}`\
\>\
\> _Example_\
...
\> _Usage_`{{UserEmail|convertToSHA256NoSalt()}}`\
\>\
\> _Example_\
\> Say you want to encrypt User's Email ID on MoEngage using SHA256\
