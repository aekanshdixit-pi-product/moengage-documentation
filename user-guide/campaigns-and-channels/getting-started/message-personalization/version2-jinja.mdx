...
```text
Usage (For a MM/DD/YYYY in EST Timezone)_ 
{{'%m/%d/%Y'|today('EST')}}
```
...
| Operator | Description |
| --- | --- |
| in | Perform a sequence/mapping containment test. Returns true if the left operand is contained in the right.  would, for example, return true. |
