...
- On Android 14 and above devices, the default `"Allow only while using the app"` location permission blocks geofence and location-triggered pushes when the app is closed or in the background. Change it `"Allow all the time"` so that location-triggered campaigns run in all states.
