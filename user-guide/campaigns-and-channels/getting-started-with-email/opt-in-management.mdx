...
sidebarTitle: "Opt-in Management"
---
...
- [Single Opt-in](#h_01JQR09AXX8TXEWSDGP1MAJDHM)
- [Double Opt-in](#h_01JQR42AZB39G5SSND4YM6XHBJ)
...
- The regulations of some regions, aiming for higher engagement quality, highlight the significance of [Double Opt-in](#h_01JQR42AZB39G5SSND4YM6XHBJ).
</Info>
...
You can access the Double Opt-in method from the **Subscription Management** \> **Opt-in Management** tab of [Email settings](https://help.moengage.com/hc/en-us/articles/360059172811-Configure-Email-Channel#h_01J1SPPGFCPX2CW998XQQR205R).
...
- Trigger the Track User Data API of MoEngage to update the opt-in status. For more information, refer to [Track User](https://developers.moengage.com/hc/en-us/articles/4413167462804-Track-User).
- Update the Opt-in status using the [user imports capability](https://help.moengage.com/hc/en-us/articles/19192630201620-Import-User-Data-Using-CSV) of MoEngage. Perform the following steps to upload the opt-in via CSV upload for your users:
  ...
  2. On the **User Imports** page, click **Import users** in the upper-right corner.  
      <img alt="import user 1.png" src="/images/moengage_8971b1.png" />
  ...
  5. Click upload from the computer in the **Select file** section to upload the CSV file. Make sure to follow the points below while uploading the CSV file. In the CSV file to be uploaded:
     ...
     - Maintain another column for the user's opt-in status that needs to be updated. The values of the Opt-in status can be *Optin pending*, *Double opted in*, or *Opted out,* and they are case in-sensitive.  
        <img alt="import user 2.png" src="/images/moengage_368082.png" />
  6. After uploading the file, click **Preview file** to preview the uploaded file. Click **Next**.  
      <img alt="Screenshot 2025-04-21 at 9.59.17 PM.png" src="/images/moengage_091328.png" />
  7. Under **Map columns**, map your *Identifier* column (in this example, the Email ID) to **ID** and Opt-in status column to the *Email Opt-in Status* attribute. The categories are preloaded in a **String** format.  
      <img alt="Screenshot 2025-04-21 at 9.09.59 PM.png" src="/images/moengage_856c97.png" />
  ...
  11. On the following screen, select the **Import frequency**. Click **Schedule Import**.  
       <img alt="Screenshot 2025-04-21 at 9.13.32 PM.png" src="/images/moengage_105737.png" />
  12. After the sync is successful, navigate to User Profiles to verify the same. Perform the following steps to access user profiles:  
      ...
      4. In the User Profile, you can view the **Email Opt-in Status** in the **Email Suppression Type**.  
          <img alt="opt-in status.png" src="/images/moengage_340acf.png" />
...
1. **Create a Landing Page**: This page helps capture the user's consent.
   - **Landing page is enabled for your account**: Create a landing page in the MoEngage dashboard. For more information, refer to [Create a Landing Page](https://help.moengage.com/hc/en-us/articles/32026262347028).
   - **Landing page is not enabled for your account**: Create your own landing page to collect the double opt-in preference of the user. Trigger the [Email Opt-in Management API of MoEngage](https://developers.moengage.com/hc/en-us/articles/36398727638932-Email-Optin-Management-API) to pass this information back to MoEngage.
2. **Create a Periodic Flow campaign**: This helps send follow-up emails asking the users for explicit permission. While creating the periodic flow campaign, define the following:
   1. In step 2, **When will users enter the flow**, define the flow entry condition.
      ...
      2. Under **Periodic**, click **Daily**. **<img alt="step 2 periodic flow.png" src="/images/moengage_ec6b37.png" />**
   2. In step 3, **Who will enter the flow**, define the target audience.
      ...
      7. Select **First Seen** as before the day you configured Double opt-in (in the image below, consider the double opt-in is configured on April 12, 2025). For more information, refer to [Create a Flow](https://help.moengage.com/hc/en-us/articles/26100424636436-Create-a-Flow).  
          <img alt="Screenshot 2025-04-21 at 11.55.11 PM.png" src="/images/moengage_6031c7.png" />
   3. Define the series of communication at a frequency of your choice.

<Accordion title="rm and Landing Page are enabled">

**Perform the following steps to update the opt-in status of new users:**

1. **Create a Landing Page**: This page helps capture the user's consent. Create a landing page in the MoEngage dashboard. For more information, refer to [Create a Landing Page](https://help.moengage.com/hc/en-us/articles/32026262347028-Create-a-Landing-Page).
2. **Create an email alert**: This alert helps deliver the consent-seeking email when the user signs up. Make sure to follow the points below while [creating an email alert](https://help.moengage.com/hc/en-us/articles/19094285198484-Create-an-Alert):
...
3. **Create a Connector campaign**: This helps update the opt-in status of users submitting preferences on the created landing page by making an API call to the [Email Optin Management API](https://developers.moengage.com/hc/en-us/articles/36398727638932-Email-Optin-Management) of MoEngage. Perform the following steps to create a connector campaign:
...
4. **Create a Periodic Flow campaign**: This helps send follow-up emails asking the users for explicit permission. While creating the periodic flow campaign, make sure to define the following:
   ...
   3. Under **Periodic**, click **Daily**.  
       <img alt="step 2 periodic flow.png" src="/images/moengage_ec6b37.png" />
   ...
   10. Click the **User property** tab and select the **Email Opt-in Status** attribute as not equal to *Opted out*. For more information, refer to [Create a Flow](https://help.moengage.com/hc/en-us/articles/26100424636436-Create-a-Flow).  
        <img alt="Screenshot 2025-04-22 at 10.39.24 AM.png" src="/images/moengage_679c5d.png" />
...
<Accordion title="rm is enabled, and Landing Page is not enabled">

Perform the following steps to update the opt-in status of new users:

1. **Create a Landing Page**: This page helps capture the user's consent. Create your own landing page to collect the user's double opt-in preference. Trigger the [Email Opt-in Management API of MoEngage](https://developers.moengage.com/hc/en-us/articles/36398727638932-Email-Optin-Management-API) to pass this information back to MoEngage.
...
<Accordion title="rm is not enabled, and Landing Page is enabled">
...
3. **Create a Connector campaign**: This helps update the opt-in status of users submitting preferences in the created landing page by making an API call to the [Email Optin Management API](https://developers.moengage.com/hc/en-us/articles/36398727638932-Email-Optin-Management-API) of MoEngage. Make sure to follow the points below while creating a connector campaign:
   1. Select the delivery type as **Event Triggered**.
   2. In step 1, **Target users**, define the **Trigger criteria** as users who have executed the*Response Submitted* event with the attribute **Landing Page Id***.  
       <img alt="Screenshot 2025-04-22 at 10.19.35 AM.png" src="/images/moengage_113ed2.png" />*
   3. Under **Select audience**, click **All users**.
...
<Accordion title="rm and Landing Page are not enabled">
...
The [Double Opt-in](https://help.moengage.com/hc/en-us/articles/360059172811-Configure-Email-Channel#opt-in-management) option of MoEngage allows you to enable or disable the **Send only to double-opted-in users** toggle based on these regional considerations while creating the email campaign. This allows for a tailored approach to email list management. For more information, refer [here](https://help.moengage.com/hc/en-us/articles/360059171891-Create-an-Email-Campaign#double_optin).
...
<Accordion title="If I track email IDs across multiple user attributes, how will MoEngage identify the users to update the opt-in value when we pass the same through the [Email Opt-in Management API](https://developers.moengage.com/hc/en-us/articles/36398727638932-Email-Optin-Management-API)?">
...
- Anytime the users' opt-in status gets updated using the [Email Opt-in Management API](https://developers.moengage.com/hc/en-us/articles/36398727638932-Email-Optin-Management-API), [Resubscribe API](https://developers.moengage.com/hc/en-us/articles/4405019553428-Resubscribe-Email-API), [Update Subscription Preference API](https://developers.moengage.com/hc/en-us/articles/11169263815828-Update-Subscription-Preferences-API), or via the end-users' interaction, MoEngage raises an event called *Email Optin Status Updated* with the necessary information.  
  **Note**: The retention of this event would be the same as any other event tracked in MoEngage. If you want to store this for a longer period, use Streams or other export options provided by MoEngage to store it on your side. Click here to know [more](https://help.moengage.com/hc/en-us/sections/16099383091348-Exports).
