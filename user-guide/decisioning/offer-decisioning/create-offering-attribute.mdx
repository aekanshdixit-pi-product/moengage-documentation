...
| Decrease Max Inputs Allowed (Fixed only) | Conditional | Blocked if any existing offering uses an attribute with more options than the new limit. For example, to lower a limit to 2, all current offerings must already have 2 or fewer options. |