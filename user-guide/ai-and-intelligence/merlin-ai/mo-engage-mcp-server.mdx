...
| Tool | Description |
| :--- | :--- |
| `search_email_campaigns` | - Search Email campaigns by name, status, or ID.<br/>- Returns campaign details including name, status, delivery type, and timestamps. |
| `search_push_campaigns` | - Search Push notification campaigns by name, status, or ID.<br/>- Returns campaign details including name, status, delivery type, and timestamps. |
| `search_sms_campaigns` | - Search SMS campaigns by name, status, or ID.<br/>- Returns campaign details including name, status, delivery type, and timestamps. |
...
![MCP request access.png](/guide-media/01KNP5QQAC13P91GXCVTQYFF7C)
...
<Tabs>
  ...
  <Tab title="Connect with Claude">
    ...
    **Information**
    ...
    8. Configure **Tool permissions** to control when Claude can use each tool - choose between *Automatic, Ask first,* or *Disabled* for each tool.
