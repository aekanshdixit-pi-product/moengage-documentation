...
description: "Personalize MoEngage landing pages with user attributes, events, product sets, and content APIs by typing @ references inside the page editor."
---
...
4. In the **Choose Fallback** section, you can decide what to display if the specified UTM parameter is missing from the page URL or has no value:
   ...
   1. **Replace with user attribute (Optional)**: To fallback to a specific user attribute value, select the desired user attribute from the list.
...
Once you have published the landing page with personalized elements, you can use the page's URL across other channels. The URL will consist of the domain name, landing page path, and a query parameter starting with *moe\_aid*, which will include the user attribute.  
