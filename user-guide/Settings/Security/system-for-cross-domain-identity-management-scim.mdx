...
- Complete the SSO configuration before enabling SCIM. For more information, refer to [Single Sign-On (SSO)](https://help.moengage.com/hc/en-us/articles/360035238511-Single-Sign-On-SSO#h_01HCHJ2TZP7NY5XYH9P93DDNAR).
</Tip>
...
4. Click the application name from your list that you want to configure SCIM for.  
    ...
    <img  src="/images/moengage_cfa82d.png"  />
...
4. Turn the **Enable SCIM Provisioning** toggle on.  
    ...
    <img   src="/images/moengage_db66a7.png"   />
5. Copy the **Base Connector URL** to use in [Step 4](#h_01KHAWN276X9M7BZM1J7RS57FA).
...
8. Click **Generate Access Token**.
   ...
   - **Note**: Copy the token immediately; it is only shown once.  
      <img  src="/images/moengage_51c118.png"  />
9. Click **Okay**.  
    <img   src="/images/moengage_de3c06.png"   />
...
3. Enter the connection details:
   1. **SCIM connector base URL**: Paste the URL copied from the MoEngage workspace in [Step 3](#h_01KHAWN276SFTZWFER31R00G1K).
...
6. In the **HTTP Header** section, in the **Authorization** box, paste the generated access token copied in [Step 3](#h_01KHAWN276SFTZWFER31R00G1K) from the MoEngage workspace.
7. Click **Test Connector Configuration**.  
     
    <img   src="/images/moengage_16e62d.png"   />   
...
8. Click **Close** and then **Save.**  
    <img   src="/images/moengage_d46f81.png"   />   
     
    <img   src="/images/moengage_792899.png"   />
...
3. Select the **Enable** check box for the following actions:
   ...
   - **Deactivate Users**: Revoke MoEngage access immediately when a user is unassigned in Okta.  
      <img   src="/images/moengage_34a867.png"   />
...
- **Default Role**: If no roles are passed via SCIM, the system uses the default role configured in the MoEngage workspace ([Step 3](#h_01KHAWN276SFTZWFER31R00G1K)).
</Info>
...
2. On the left navigation menu, click **Directory** \> **Profile Editor**.  
    **<img  src="/images/moengage_8b3c03.png"   />**
3. On the **Profile Editor** page, find your application in the list or use the **Search for people, apps, and groups** box to find and select the appropriate application from your list.  
    <img  src="/images/moengage_b6ae58.png"   />
4. Under the **Attributes** section, click **+ Add Attribute**.  
    <img  src="/images/moengage_f75ddd.png"  />   
...
5. In the **Add Attribute** dialogue box, configure the following details:
   ...
   5. In the **External namespace,** enter *urn:ietf:params:scim:schemas:extension:moengage:2.0:User*  
       <img  src="/images/moengage_4f66ba.png"  />
...
7. Click **Save**.  
    <img  src="/images/moengage_1de290.png"  />   
...
5. After the group is created, click it and copy the Group ID from the browser's URL (this is the unique identifier required for mapping logic).  
    <img  src="/images/moengage_55631c.png"  />

### Step 6.2.2: Assign Users

1. Within the **Groups** page, click **Assign People**.  
    <img  src="/images/moengage_02be42.png"  />
...
2. Click the **Okta** filter and click the **Okta User (default)** profile.  
    <img  src="/images/moengage_197455.png"  />
3. Scroll to the end of the page and verify whether the role attribute created in [Step 6.1](#h_01KHNG67JVC2D75V7NRSP7EJHH) exists. If the attribute is missing, click **+ Add Attribute**.
4. In the **Add Attribute** dialog box, add a role attribute with the following settings:
   ...
   4. In the **Description** (optional) box, enter a description for the role.  
       <img   src="/images/moengage_6f78f6.png"   />
   5. In the **User permission** section, select the **Read-Write** radio button.
5. Click **Save**.  
    <img   src="/images/moengage_a0704d.png"   />
...
5. In the **expression** box, enter the role attribute following logic to map Okta groups to MoEngage roles (isMemberOfGroup("group\_id") ? "Admin" : "User").
   1. **group\_id**: Paste the ID copied in [Step 6.2.1](#h_01KHNB4F3MDT9M7Q7X7M8CVN9R).
   2. **Role**: The specific role name as defined in your MoEngage workspace.
6. Confirm and paste that the role attribute details (namespace and external name) for the role attribute match the mandatory values defined in [Step 6.1](#h_01KHNAXKPG8MVHVYDHQZR37HE1).
7. Click **Save Mappings** and then click **Apply updates**.  
    <img   src="/images/moengage_5c332a.png"   />
...
2. Click **+ Push Groups** \> **Find groups by name**.  
    <img   src="/images/moengage_de1858.png"   />
...
4. Ensure the **Push group memberships immediately** check box is selected.  
    <img   src="/images/moengage_083657.png"   />
...
3. Click **Force Sync**.  
    <img  src="/images/moengage_27e19e.png"   />   
...
- **User Invitation**: Confirm the user is successfully invited and the status is Joined.  
   <img  src="/images/moengage_0fa0d5.png"   />
- **Default Role Update**: Confirm that if no specific role was passed from Okta, the system correctly assigned the Default Role configured in [Step 3](#h_01KHAWN276SFTZWFER31R00G1K).
- **Role Accuracy**: Confirm that the specific role sent from Okta (based on group membership logic) matches the assigned role in the workspace.  
   <img  src="/images/moengage_be5a3f.png"   />
...
1. Before enabling SCIM, ensure you have successfully set up the SSO connection between MoEngage and Azure. For more information, refer to [Single Sign-On (SSO)](https://help.moengage.com/hc/en-us/articles/360035238511-Single-Sign-On-SSO).
2. To verify your enterprise application,
   ...
   4. On the **Enterprise applications | All applications** page, find your application in the list or use the **Search by application name or object ID** box to find it and select the application name you want to configure from your list.  
       <img  src="/images/moengage_9e6764.png"   />
...
2. On the **Security** page, click the **Login** tab.
3. Click **Single Sign On (SSO) only**.
4. Under **Single Sign On (SSO) only**, turn the **Enable SCIM Provisioning** toggle on.  
    <img  src="/images/moengage_3ab29c.png"   />
5. Copy the **Base Connector URL** and save it (to use in [Step 3](#h_01KEHS1NNS9C08YYGTPVZ048DF)).
6. In the **Default role** list, select the required role.
7. In the **Expiry Date** field, select the expiry date for the SCIM access token.  
   **Note**: MoEngage recommends setting an **Expiry date** period of one year.
8. Click **Generate Access Token**.  
    <img  src="/images/moengage_5974c0.png"  />   
   The **generated access token** is displayed.  
   **Note**: You can view the access token only once; ensure to copy it.
9. Click **Okay**.  
    <img  src="/images/moengage_882217.png"   />
...
2. In the **Provisioning Mode**, click **Automatic**.  
    <img  src="/images/moengage_f3e7fc.png"  />
3. Expand **Admin Credentials** and enter the following:
   ...
   2. In the **Secret Token** box, paste the **SCIM Access Token** generated in [Step 2.1](#h_01KEHRB7X2HN8RRW742ZXJ8ZK0).
   ...
   4. Click **Save** at the top.  
       <img  src="/images/moengage_22aa54.png"  />
...
5. Click **Save** at the top of the pane.  
    <img  src="/images/moengage_350ca4.png"  />
...
1. On the **Attribute Mapping** page, click **Add New Mapping.**  
    **<img  src="/images/moengage_cea83c.png"   />**
2. In the **Edit Attribute**section, configure the following details:
   ...
   3. In the **Target Attribute**list, click the attribute that you created in [Step 4.1](#h_01KEJ1J33002MSXHF9DAWC23M9).
   4. In the **Apply this mapping**list, click **Always**.
3. Click **OK**.  
    <img  src="/images/moengage_488f84.png"   />
4. Click **Save** at the top.
...
2. Click the **All Applications** tab and select your application.  
    <img  src="/images/moengage_9a819c.png"   />
3. In the left navigation menu, click **Manage** \> **App roles**.
4. Click the **+ Create app role**.  
    <img  src="/images/moengage_ef2561.png"   />
5. Configure the following details:
   ...
   6. Click **Apply** at the bottom of the pane.  
       <img  src="/images/moengage_a5d2e9.png"   />
...
1. Navigate to **Home** \> **Microsoft Entra ID** \> **Manage** \> **Enterprise applications** and select your application.  
    <img  src="/images/moengage_9e6764.png"   />
2. In the **Getting started** section, in the **Assign users and groups** tile, click the **Assign users and groups** link.  
    <img  src="/images/moengage_9bdfa4.png"   />
3. Select the check box for the user you want to update and click the **Edit assignment** icon <img  src="/images/moengage_29a800.png"   /> in the top menu bar.  
    <img  src="/images/moengage_f84f85.png"  />
4. On the left side, under **Select a role**, click the **None Selected** link.  
   The **Select a role** dialog box pane appears on the right side. A list appears displaying the custom App Roles created in [Step 5.1.](#h_01KEM4BMXP1M3A0GC7JZYVH82K)
5. Select the appropriate role from the list and click **Select**.  
   **Note**: Ensure you do not leave the role as Default Access. Microsoft Entra ID requires a specific role value to pass the correct permission data to MoEngage during the SCIM synchronization.  
    <img src="/images/moengage_408b82.png" />
6. Click **Assign** at the bottom of the **Edit Assignment** page.  
    <img  src="/images/moengage_c8824c.png"   />   
...
7. To verify the assignment, click the user's name to open their **Overview** page. The **Basic info** section displays the user's **User principal name** and confirms that the number of **Assigned roles** is now updated.  
    <img  src="/images/moengage_c26bf6.png"  />
...
1. Return to the Azure application, on the **Overview** page, go to the left navigation menu, and click **Manage** \> **Provisioning**.
2. In the top menu bar, click **Provision on demand**.  
    <img  src="/images/moengage_1fb5d9.png"   />
3. In the **Search for a user or group** box, type the name or email address of the user you assigned in [Step 5.2](#h_01KEM5FTKDHKXRQKKFBBW4D3G5).
...
5. Click **Provision** at the bottom of the page. Microsoft Entra ID initiates an immediate "force push" of the user identity, including the custom MoEngage role mapping.  
   **Note**: After the process completes, a success message appears with details of the exported attributes. The SCIM status in your MoEngage workspace will now reflect these updates.  
    <img  src="/images/moengage_ea88ef.png"   />
...
1. **User Invitation**: Confirm the user is successfully invited and the status is Joined.  
    <img  src="/images/moengage_0fa0d5.png"   />
2. **Default Role Update**: Confirm that if no specific role was passed from Microsoft Entra ID, the system correctly assigned the Default Role configured in [Step 3](#h_01KEHS1NNS9C08YYGTPVZ048DF).
