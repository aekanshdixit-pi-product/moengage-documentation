...
description: Access MoEngage documentation in formats suited for AI agents, LLMs, and automation. Use the MCP server, per-page markdown, and optional llms.txt-style endpoints for lower token usage than full web pages.
icon: robot
layout: guide
---
...
* **AI coding assistants (Claude Code, Cursor, Windsurf):** Use the [MoEngage Docs MCP server](https://www.moengage.com/docs/mcp) for the best experience. It lets your assistant search and fetch documentation from within the IDE.
...
* **Project rules and custom instructions:** Add a section-level `llms.txt` URL to your AI tool’s rules so it has an index of relevant MoEngage pages.
...
* **API-focused development:** Attach the [OpenAPI spec](#openapi-specification) to your AI tool’s context so it knows the full API surface when writing integration code.

## MCP server

The [MoEngage MCP server](/mcp) gives AI agents and coding assistants direct access to MoEngage documentation and references through the Model Context Protocol. Use it to bring MoEngage knowledge into tools like Claude, Cursor, and other MCP-compatible clients.

The MoEngage MCP (Model Context Protocol) server connects your AI agents to MoEngage’s docs and developer resources. Configure the MoEngage MCP server in your IDE or AI chat to get in-context help for your MoEngage integration.
...
To set up the MoEngage MCP server, pick your IDE or AI chat provider and follow the steps below.
...
<AccordionGroup>
  ...
  <Accordion title="Windsurf">
    <Note>
      Windsurf uses `npm` to run the MoEngage MCP server. You need **Node.js** installed. See [Node.js downloads](https://nodejs.org/en/download/) for details.
    </Note>

    1. Open **Windsurf**.

    2. Open the *Cascade assistant* sidebar.

    3. Click the **hammer (MCP)** icon to open MCP settings.

    4. Click **Configure > View raw config**.

    5. Open `~/.codeium/windsurf/mcp_config.json`. This file is Windsurf’s global MCP configuration.

    6. Add the following to `mcp_config.json`:
    ...
    8. In the Cascade sidebar, click **Refresh** next to "available MCP servers".
  </Accordion>
...
If your docs deployment supports the [llms.txt standard](https://llmstxt.org/), you can use these endpoints for markdown-formatted content.
...
For narrower scope, use section-level endpoints. The `llms.txt` variant is a page index with descriptions; `llms-full.txt` is full page content.

### Developer guide

SDK setup, integration guides, and platform-specific documentation.

| Endpoint | Description | Size |
| -------- | ----------- | ---- |
| [`/developer-guide/llms.txt`](https://www.moengage.com/docs/developer-guide/llms.txt) | Page index for the developer guide | Varies |
| [`/developer-guide/llms-full.txt`](https://www.moengage.com/docs/developer-guide/llms-full.txt) | Full content of the developer guide | Varies |

### API reference

API references, request/response shapes, and developer-facing API docs.

| Endpoint | Description | Size |
| -------- | ----------- | ---- |
| [`/api-reference/llms.txt`](https://www.moengage.com/docs/api-reference/llms.txt) | Page index for the API reference | Varies |
| [`/api-reference/llms-full.txt`](https://www.moengage.com/docs/api-reference/llms-full.txt) | Full API reference content. May include or link to the OpenAPI spec | Varies |

## skill.md

A `skill.md` file at the root of the MoEngage docs site describes what AI agents can do with MoEngage in a structured, machine-readable format. It makes capabilities, required inputs, and constraints explicit so agents can use your product more reliably.
...
* **Context** — High-level overview of MoEngage’s architecture and concepts.
