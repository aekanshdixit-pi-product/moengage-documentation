...
<Info>
Authentication is performed using **Basic Auth** (Workspace ID and Data API Key). You must also provide the `MOE-APPKEY` in the request headers.
</Info>
...
<Note>
**Archive Policy:**
Archived segments are stored for retrieval and reuse, but will not be visible or accessible beyond **180 days**.
</Note>

<Note>
**CSV Requirements:**
- Must be a single column with no empty rows or columns.
- Values should not contain special characters (e.g., `"abcd"` or `a#bc`).
- Each value must uniquely identify a single user in MoEngage.
</Note>

<Warning>
If your file storage is private, ensure you whitelist MoEngage IPs to allow the system to download and process your CSV files.
</Warning>
...
---
Would you like me to help you format the specific request/response examples for these endpoints?