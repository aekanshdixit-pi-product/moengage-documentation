...
For a complete technical reference, check out our [Javascript Documentation][Placeholder_Link] or our sample apps for [LG TV](https://github.com/moengage/webSDK-sample/tree/master/lg-web-os)  and [Samsung TV](https://github.com/moengage/webSDK-sample/tree/master/samsung-tizen-os/samsungTizenOs) to see the Web SDK running on a TV.

<Check>
1. Do not add the serviceworker.js file, as Push Notification does not work here.
2. Pass `disable_web_push: true` to the initialization config., so that the SDK does not initialize the Push Notification module. More details are available [here.][Placeholder_Link]
3. Pass `disableCookies: true` to disable the usage of any cookies
</Check>

## Supported Features

* [Data Tracking][Placeholder_Link]
* [On-site Messaging][Placeholder_Link]
