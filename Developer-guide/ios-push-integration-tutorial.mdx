...
<Info>
  **Prerequisites**
  ...
  1. **Integrate the MoEngage iOS SDK**: Use CocoaPods, Swift Package Manager (SPM), or manual framework import as detailed in the [MoEngage documentation](https://developers.moengage.com/hc/en-us/sections/6993218591636-Basic).
  2. **Configure APNs credentials in the MoEngage UI**:
      * MoEngage requires either an APNs Authentication Key (.p8) or an [APNs Certificate](https://developers.moengage.com/hc/en-us/articles/4403944011028-APNS-Certificate-PEM-file) (.pem) to communicate with Apple Push Notification Service (APNs).
      * **Recommendation**: Use an APNs Authentication Key because it is non-expiring. Refer to the [APNs Authentication Key Setup Guide](https://developers.moengage.com/hc/en-us/articles/8484447635348-APNS-Authentication-Key#apns-certificate--0-0).
...
<Steps>
  ...
  <Step title="Integrate MoEngageRichNotification">
    <Tabs>
      ...
      <Tab title="CocoaPods">
        <Warning>
          **Information**
          CocoaPods is being deprecated. MoEngage recommends using Swift Package Manager for all new integrations. For more info, refer [here](https://developers.moengage.com/hc/en-us/articles/4403900743828-SDK-Integration).
        </Warning>
  ...
  <Step title="Set up App Group">
    ...
    3. **Update AppDelegate**:
       Modify your `AppDelegate.swift` in your iOS application to set the App Group ID as shown below:
...
<Tabs>
  <Tab title="Provisional + Standard Push">
    <Tabs>
      <Tab title="Swift">
        ```swift
        import UIKit
...
<Accordion title="How to resolve App Delegate Swizzling conflicts?">
  ...
  2. **Manual forwarding in AppDelegate**:
     After disabling swizzling, implement the following code within your `AppDelegate.swift` file to manually pass notification events to the MoEngage SDK:
...
For more information, refer to [iOS Push Notifications Integration Validation](/hc/en-us/articles/360052433591).
